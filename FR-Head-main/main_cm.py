...
class Processor:
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            ...
            #     # Print or examine the data to check if it is as expected
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                ...
                with torch.no_grad():
                    ...
                    print(calc_diff_modality(data, **self.test_modality).shape)
            ...
            labels = ['Hold', 'Slide_H', 'PDown', 'PUp', 'Roll_T', 'Rotate', 'Screw', 'Lock', 'Press', 'Shake', 'Open', 'Close', 'Clean', 'Flip', 'Attach', 'Detach']
            filename = 'cm_1.pdf'
            # print(label_list)
            y_true = []
            y_pred = []
            for i in range(len(label_list)):
                y_true.append(labels[label_list[i]])
                y_pred.append(labels[pred_list[i]])
            ...
            # sns.heatmap(cm, annot=annot, fmt='', ax=ax, cbar=False, cmap="crest")
